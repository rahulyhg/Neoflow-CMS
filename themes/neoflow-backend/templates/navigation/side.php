<nav class="navbar navbar-expand-lg navbar-dark bg-secondary" id="sideNavigation">
    ...
    <div class="sidenav-content">
        <ul class="list-unstyled">
            <li>Version <?= $view->settings()->getVersion(); ?></li>
        </ul>
        <ul class="list-unstyled small">
            ...
            <li>
                <?= translate('Session timeout in {0}', array('<span class="timer" id="sessionTimer" data-timeout-callback="showReloginModal()" data-time="'.$view->config()->get('session')->get('lifetime').'">'.date('H:i:s', $view->config()->get('session')->get('lifetime')).'</span>'), true, false); ?>
            </li>
            <li>
                <?= translate('Page loaded in {0} seconds', [round(\Neoflow\CMS\App::instance()->getExecutionTime(), 3)]); ?>
            </li>
        </ul>
    </div>
</nav>