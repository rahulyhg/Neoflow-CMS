        ...
        if ($navitems->count()) {
            ...
            foreach ($navitems as $navitem) {
                                    ...
                                    <td>
                                        <ul class="list-inline">
                                            <li class="list-inline-item">
                                                <a href="'.generate_url('backend_navitem_edit', array('id' => $navitem->id())).'" title="'.translate('Edit item').'">';
                ...
                if (1 != $navitem->navigation_id) {
                    $output .= '<a href="'.generate_url('backend_navitem_edit', array('id' => $navitem->id())).'" class="btn btn-outline-light btn-sm d-none d-xl-inline-block btn-icon-left" title="'.translate('Edit item').'">
                                        <span class="btn-icon">
                ...
                if ($navitem->is_active) {
                    $output .= ' <a href="'.generate_url('backend_navitem_toggle_activation', array('id' => $navitem->id())).'" class="btn btn-outline-light btn-sm confirm-modal" data-message="'.translate('Are you sure you want to disable it?').'" title="'.translate('Disable item').'">
                                    <i class="fa fa-fw fa-toggle-on"></i>
                                </a> ';
                } else {
                    $output .= ' <a href="'.generate_url('backend_navitem_toggle_activation', array('id' => $navitem->id())).'" class="btn btn-outline-light btn-sm confirm-modal" data-message="'.translate('Are you sure you want to enable it?').'" title="'.translate('Enable item').'">
                                    <i class="fa fa-fw fa-toggle-off"></i>
                                </a> ';
                }

                if (1 != $navitem->navigation_id) {
                    $output .= '<a href="'.generate_url('backend_navitem_delete', array('id' => $navitem->id())).'" class="btn btn-primary btn-sm confirm-modal" data-message="'.translate('Are you sure you want to delete this element all of its subelements?').'" title="'.translate('Delete item').'">
