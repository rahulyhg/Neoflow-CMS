            ...
            <?php if (count($languages) > 1) { ?>
                <ul class="nav nav-tabs">
                    <?php foreach ($languages as $language) { ?>
                        <li class="nav-item">
                            <a class="nav-link<?= ($language->id() === $activeLanguage->id() ? ' active' : '') ?>"
                               href="<?= generate_url('backend_page_index', ['language_id' => $language->id()]) ?>">
            ...
            <div class="card-body">
                <?php if ($navitems->count()) { ?>
                    <div class="nestable" data-save-url="<?= generate_url('backend_page_reorder') ?>">
                        <?= $view->renderTemplate('backend/page/nestable', [
                            'navitems' => $navitems,
                            'view' => $view
                        ]) ?>
                    </div>
