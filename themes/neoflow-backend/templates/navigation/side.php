    ...
    <div class="collapse navbar-collapse" id="sidenavCollapse">
        <ul class="navbar-nav navbar-sidenav">
            ...
            <?php if (has_permission('manage_pages') || has_permission('manage_navigations') || has_permission('manage_blocks')) { ?>
                <li class="nav-item<?= is_current_route([
                    'backend_navigation*', 'backend_page*', 'backend_section*', 'pmod_*', 'backend_navitem*', 'backend_block*'
                ], ' active'); ?>">
                    <a href="#content-menu" class="nav-link nav-link-collapse<?= is_current_route([
                        'backend_navigation*', 'backend_page*', 'backend_section*', 'pmod_*', 'backend_navitem*', 'backend_block*'
                    ...
                    <ul id="content-menu" class="sidenav-second-level collapse<?= is_current_route([
                        'backend_navigation*', 'backend_page*', 'backend_section*', 'pmod_*', 'backend_navitem*', 'backend_block*'
                    ], ' show'); ?>">
            ...
            if (has_permission('manage_modules') || has_permission('manage_themes')) {
                ...
                <li class="nav-item<?= is_current_route(['backend_module*', 'backend_theme*'], ' active'); ?>">
                    <a href="#extension-menu"
                       class="nav-link nav-link-collapse<?= is_current_route(['backend_module*', 'backend_theme*'], '', ' collapsed'); ?>"
                    ...
                    <ul id="extension-menu"
                        class="sidenav-second-level collapse<?= is_current_route(['backend_module*', 'backend_theme*'], ' show'); ?>">
            ...
            if (has_permission('manage_users') || has_permission('manage_roles')) {
                ...
                <li class="nav-item<?= is_current_route(['backend_user*', 'backend_role*'], ' active'); ?>">
                    <a href="#account-menu"
                       class="nav-link nav-link-collapse<?= is_current_route(['backend_user*', 'backend_role*'], '', ' collapsed'); ?>"
                       data-toggle="collapse">
                        <span class="nav-link-icon">
    ...
    <div class="sidenav-content">
        ...
        <ul class="list-unstyled small">
            ...
            <li>
                ...
                $sessionLifetime = $view->config()->get('session')->get('lifetime');
                $timer = '<span class="timer" id="sessionTimer" data-timeout-callback="showReloginModal()" data-time="' . $sessionLifetime . '">' . gmdate('H:i:s', $sessionLifetime) . '</span>';

                echo translate('Session timeout in {0}', [
                    $timer
                ], true, false); ?>
            </li>
            <li>
