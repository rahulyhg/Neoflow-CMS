<?php

return [
    'translation' => [
        'Page module with an article system for blogs and news.' => 'Seitenmodul mit einem Artikelsystem für Blogs und News.',
        'Articles per page' => 'Artikel pro Seite',
        'Blog' => 'Blog',
        'Article' => 'Artikel',
        'Number of articles' => 'Artikelanzahl',
        'Abstract' => 'Kurzfassung',
        'All articles' => 'Alle Artikel',
        'Delete article' => 'Artikel löschen',
        'Edit article' => 'Artikel bearbeiten',
        'Create article' => 'Artikel erstellen',
        'Category' => 'Kategorie|Kategorien',
        'All categories' => 'Alle Kategorien',
        'Delete category' => 'Kategorie löschen',
        'Edit category' => 'Kategorie bearbeiten',
        'Create category' => 'Kategorie erstellen',
        'Only required if the description of the website should contain a different description than that of the category (e.g. because of SEO).' => 'Wird nur benötigt, wenn die Beschreibung der Webseite eine andere als die der Kategorie beinhalten soll (zB. wegen SEO).',
        'Only required if the title of the website should contain a title other than that of the category (e.g. because of SEO).' => 'Wird nur benötigt, wenn der Titel der Webseite einen anderen als den der Kategorie beinhalten soll (zB. wegen SEO).',
        'Only required if the description of the website should contain a different description than that of the article (e.g. because of SEO).' => 'Wird nur benötigt, wenn die Beschreibung der Webseite eine andere als die des Artikels beinhalten soll (zB. wegen SEO).',
        'Only required if the title of the website should contain a title other than that of the article (e.g. because of SEO).' => 'Wird nur benötigt, wenn der Titel der Webseite einen anderen als den des Artikels beinhalten soll (zB. wegen SEO).',
    ],
];
