            ...
            <div class="card-body">
                ...
                <form method="post" action="<?= generate_url('backend_role_update') ?>">
                    ...
                    <div class="form-group row <?= has_validation_error('description', 'is-invalid') ?>">
                        ...
                        <div class="col-sm-9">
                            <textarea name="description" class="form-control vresize" maxlength="150" id="textareaDescription"
                        ...
                        <div class="col-sm-9">
                            <select multiple class="form-control" name="permission_ids[]" id="selectPermissions" data-placeholder="">
                                <?php foreach ($permissions as $permission) { ?>
                                    <option value="<?= $permission->id() ?>" <?= (in_array($permission->id(), $role->permission_ids) ? 'selected' : '') ?>
                                            data-description="<?= translate($permission->description, [], true) ?>"><?= translate($permission->title, [], true) ?></option>
                                <?php }
                                ?>
                            </select>
                        </div>
