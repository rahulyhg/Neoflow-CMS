                ...
                foreach ($modules as $module) {
                    ...
                    <tr class="<?= ($module->is_active ?: 'table-muted') ?>">
                        ...
                        <td class="text-left-xs text-right nowrap">
                            <a href="<?= generate_url('backend_module_view', ['id' => $module->id()]) ?>"
                               class="btn btn-outline-light btn-sm btn-icon-left d-none d-xl-inline-block" title="<?= translate('Show module') ?>">
                                    <span class="btn-icon">
                            ...
                            <?php if ($module->is_active) { ?>
                                <a href="<?= generate_url('backend_module_toggle_activation', ['id' => $module->id()]) ?>"
                                   class="btn btn-outline-light btn-sm confirm-modal <?= ($module->is_core ? 'disabled' : '') ?>"
                                   data-message="<?= translate('Are you sure you want to disable it?') ?>" title="<?= translate('Disable module') ?>">
                                    <i class="fa fa-fw fa-toggle-on"></i>
                                </a>
                            <?php } else { ?>
                                <a href="<?= generate_url('backend_module_toggle_activation', ['id' => $module->id()]) ?>"
                                   class="btn btn-outline-light btn-sm confirm-modal <?= ($module->is_core ? 'disabled' : '') ?>"
                                   data-message="<?= translate('Are you sure you want to enable it?') ?>" title="<?= translate('Enable module') ?>">
                                    <i class="fa fa-fw fa-toggle-off"></i>
                                </a>
                            <?php } ?>
                            <a href="<?= generate_url('backend_module_delete', ['id' => $module->id()]) ?>"
                               class="btn btn-primary btn-sm confirm-modal <?= ($module->is_core ? 'disabled' : '') ?>"
                               data-message="<?= translate('Are you sure you want to delete it?') ?>" title="<?= translate('Uninstall module') ?>">
            ...
            <div class="dataTable_info_src">
                <ul class="list-inline small">
                    ...
                    <li>
                        ...
                        = <?= translate('At least one required module is unavailable') ?>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="col-xl-5">
            ...
            <div class="card-body">
                <form method="post" enctype="multipart/form-data" action="<?= generate_url('backend_module_install') ?>">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="file" name="package" data-allowed-File-Extensions="zip"
                                   data-allowed-File-Size="<?= ((int)ini_get('upload_max_filesize')) * 1024 * 1024 ?>" id="inputPackage" required
                                   class="form-control"/>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <a href="<?= generate_url('backend_module_reload_all') ?>" class="btn btn-primary btn-icon-left confirm-modal"
                   data-message="<?= translate('Are you sure you want to reload all modules?') ?>"
                   title="<?= translate('Reload module', [], true) ?>">
                    <span class="btn-icon">
